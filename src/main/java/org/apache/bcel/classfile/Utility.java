        ...
        try {
            switch (signature.charAt(0)) {
                ...
                case 'L': { // Full class name
                    ...
                    signature = compactClassName(signature.substring(1, index), chopit);
                    if (signature.contains(".")) {
                        org.candle.decompiler.util.Helper.getCurrentClass().getImports().add(signature);
                        return StringUtils.substringAfterLast(signature, ".");
                    }
                    return signature;
                }
                case 'S':
